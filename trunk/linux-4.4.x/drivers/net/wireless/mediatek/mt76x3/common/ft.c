	...
	OUT	PFT_FTIE_INFO		pFtInfo)
{
	...
	key_idx = pAd->ApCfg.MBSSID[pEntry->func_tb_idx].wdev.DefaultKeyId;
	...
	OUT PFT_INFO 			pFtInfo_out)
{
	...
	if (IS_EZ_SETUP_ENABLED(&pAd->ApCfg.MBSSID[pEntry->func_tb_idx].wdev) || ez_is_triband()){
		...
		FT_DerivePMKR0(pAd->ApCfg.MBSSID[pEntry->func_tb_idx].PMK, LEN_PMK,
	...
	if (pPmkR0Name){
		...
		hex_dump("pAd->ApCfg.MBSSID[pEntry->func_tb_idx].Bssid=", wdev->bssid, 6);
...
/*spec defines auth rsp should happen within 30 ms*/
	...
	GenRandom(pAd, wdev->bssid, pEntry->ANonce);
    ...
    if ((wdev->AuthMode == Ndis802_11AuthModeWPA1PSKWPA2PSK) ||
		(wdev->AuthMode == Ndis802_11AuthModeWPA1WPA2)) 
    {
        rsnie_len = pAd->ApCfg.MBSSID[ap_idx].RSNIE_Len[1];
        rsnie_ptr = &pAd->ApCfg.MBSSID[ap_idx].RSN_IE[1][0];
	}
    else
    {
        rsnie_len = pAd->ApCfg.MBSSID[ap_idx].RSNIE_Len[0];
        rsnie_ptr = &pAd->ApCfg.MBSSID[ap_idx].RSN_IE[0][0];
    }
...
{																								
	...
	if ((RTMPEqualMemory(pPeerFtIe->R0khId, pR1hkEntry->R0khId, pR1hkEntry->R0khIdLen) == FALSE) ||
		(RTMPEqualMemory(pPeerFtIe->R1khId, pAd->ApCfg.MBSSID[pEntry->func_tb_idx].wdev.bssid, MAC_ADDR_LEN) == FALSE) ||
...
{																								
	...
	/* Prepare RSNIE for outgoing frame */
	if ((pAd->ApCfg.MBSSID[pEntry->func_tb_idx].wdev.AuthMode == Ndis802_11AuthModeWPA1PSKWPA2PSK) ||
		(pAd->ApCfg.MBSSID[pEntry->func_tb_idx].wdev.AuthMode == Ndis802_11AuthModeWPA1WPA2)) 
    {
        rsnie_len = pAd->ApCfg.MBSSID[pEntry->func_tb_idx].RSNIE_Len[1];
        rsnie_ptr = &pAd->ApCfg.MBSSID[pEntry->func_tb_idx].RSN_IE[1][0];
	}

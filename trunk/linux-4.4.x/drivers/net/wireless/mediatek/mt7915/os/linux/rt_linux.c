...
void hex_dump_with_cat_and_lvl(char *str, UCHAR *pSrcBufVA, UINT SrcBufLen, INT dbgcat, INT dbg_sub_cat, INT dbglvl)
{
	...
	pt = pSrcBufVA;
	MTWF_LOG(dbgcat, dbg_sub_cat, dbglvl,
			 ("%s: %p, len = %d\n", str, pSrcBufVA, SrcBufLen));
	...
	for (x = 0; x < SrcBufLen; x++) {
		...
		MTWF_LOG(dbgcat, dbg_sub_cat, dbglvl, ("%02x ", ((unsigned char)pt[x])));
	...
	IN BOOLEAN autoSuffix)
{
	...
	if ((devType == INT_MBSSID) || (devType == INT_APCLI)) {
	...
	VOID *napi)
{
	...
	if (!skb) {
		MTWF_LOG(DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: failed to allocate sk_buff for notification\n", pNetDev->name));
		return -ENOMEM;
	}
...
inline void OS_SET_BIT(int bit, ULONG *flags)
{
	set_bit(bit, flags);
}

inline void OS_CLEAR_BIT(int bit, ULONG *flags)
{
	clear_bit(bit, flags);
}
...
UINT32 ShowMemAllocInfo(UINT show, UINT pCaller)
{
...
UINT32 ShowPktAllocInfo(UINT show, UINT pCaller)
{
	MTWF_LOG(DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_OFF,
...
static VOID MemInfoListExit(VOID)
{
	...
	memalctotal = ShowMemAllocInfo(0, 0);
	pktalctotal = ShowPktAllocInfo(0, 0);
